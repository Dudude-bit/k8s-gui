...
use crate::db::models::{User, UserProfile, AuditLog, token::RefreshToken};
...
impl AuthService {
    ...
    pub async fn login(&self, req: LoginRequest) -> Result<AuthResponse> {
        ...
        if let Ok(deleted) = RefreshToken::delete_all_for_user(&self.pool, user.id).await {
            if deleted > 0 {
                log::debug!("Deleted {} old refresh tokens for user {}", deleted, user.id);
            }
        }
    ...
    pub async fn refresh(&self, req: RefreshRequest) -> Result<AuthResponse> {
        ...
        let consumed_user_id = RefreshToken::consume(&self.pool, &token_hash).await?
            .ok_or_else(|| AppError::Authentication("Invalid or already used refresh token".to_string()))?;

        // Verify the user_id from JWT matches the one in database
        if consumed_user_id != user_id {
            return Err(AppError::Authentication("Token mismatch".to_string()));
        }
