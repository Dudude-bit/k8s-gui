...
export function PersistentVolumeList() {
  ...
  const { data: pvs = [], isLoading, refetch } = useQuery({
    queryKey: ['persistent-volumes'],
    queryFn: async () => {
      const result = await invoke<PersistentVolumeInfo[]>('list_persistent_volumes');
      return result;
    },
    enabled: isConnected,
    staleTime: 10000,
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable columns={columns} data={pvs} isLoading={isLoading} searchKey="name" />
    </div>
  );
}
