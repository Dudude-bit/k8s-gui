...
import type { AppInfo, ClusterInfo, ClusterMetrics, ClusterStats, ConfigMapInfo, ContextInfo, CronJobInfo, DaemonSetInfo, DeploymentInfo, EndpointsInfo, EventFilters, EventInfo, HelmRelease, IngressInfo, JobInfo, LicenseStatus, LogLine, ManifestResult, NamespaceInfo, NodeFilters, NodeInfo, NodeMetrics, PaymentHistoryResponse, PersistentVolumeClaimInfo, PersistentVolumeInfo, PodFilters, PodInfo, PodMetrics, PortForwardRequest, PortForwardSessionInfo, RegistryAuth, RegistryAuthStatus, RegistryImageResult, RegistryImportEntry, RegistrySearchRequest, ResourceFilters, ResourceListItem, ResourceQuery, RolloutStatus, SecretFilters, SecretInfo, ServiceFilters, ServiceInfo, StatefulSetInfo, StorageClassInfo, StreamLogConfig, UserProfile, WatchInfo } from "./types";
...
export async function startWatch(resourceType: string, namespace: string | null, labelSelector: string | null): Promise<string> {
  return invoke<string>("start_watch", { resourceType, namespace, labelSelector });
}

export async function stopWatch(watchId: string): Promise<void> {
  return invoke<void>("stop_watch", { watchId });
}

export async function listActiveWatches(): Promise<WatchInfo[]> {
  return invoke<WatchInfo[]>("list_active_watches");
}
