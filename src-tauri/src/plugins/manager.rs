...
impl PluginManager {
    ...
    fn register_builtin_plugins(&mut self) -> Result<()> {
        // No built-in plugins at the moment
        // Helm functionality is now provided directly through commands, not plugins
        Ok(())
    }
...
mod tests {
    ...
    fn test_plugin_manager_creation() {
        ...
        let _manager = PluginManager::new().unwrap();
    }
    ...
    fn test_plugin_count() {
        ...
        assert_eq!(
            count.total,
            count.kubectl + count.command + count.context_menu + count.renderer
        );
    }
}
