...
impl MetricsClient {
    ...
    pub async fn from_state(state: &AppState) -> Result<Self> {
        ...
        let user_name = kubeconfig.contexts.iter()
            .find(|c| c.name == context)
            .and_then(|c| c.context.as_ref())
        ...
        let base_url = cluster.server.as_deref()
    ...
    pub async fn get_pod_metrics(&self, namespace: Option<&str>) -> Result<Vec<PodMetrics>> {
        ...
        let result = metrics_list.items.into_iter()
            .map(|item| {
                let (total_cpu_millicores, total_memory_bytes) = item.containers.iter()
                    .fold((0.0f64, 0u64), |(cpu, mem), container| {
                        let cpu_delta = container.usage.cpu.as_ref()
                            .and_then(|s| parse_cpu_to_millicores(s).ok())
                            .unwrap_or(0.0);
                        let mem_delta = container.usage.memory.as_ref()
